...
const RegisterPage = () => {
  ...
  const setEmailStore = useSignUpStore(useShallow((state) => state.setEmail))
  const signUpStart = useSignUpStore(useShallow((state) => state.signUpStart))

  const isEmailValid = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)
  const doPasswordsMatch = password === confirmPassword
  const isFormValid =
    email && isEmailValid && username && password && confirmPassword && doPasswordsMatch

  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!isFormValid) return
    setEmailStore(email)
    signUpStart(username, email, password, confirmPassword).then(() =>
